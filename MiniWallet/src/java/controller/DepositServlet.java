    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (depositAmount < 1000) {
                request.setAttribute("errorMessage", "Số tiền nạp tối thiểu là 1,000");
                request.getRequestDispatcher("deposit.jsp").forward(request, response);
                return;
            }

            boolean isSuccess = transactionDAO.deposit(user.getId(), depositAmount);

            if (isSuccess) {
                double newBalance = user.getBalance() + depositAmount;
                user.setBalance(newBalance);
                session.setAttribute("user", user);
                request.setAttribute("successMessage", "Nạp tiền thành công");
            } else {
